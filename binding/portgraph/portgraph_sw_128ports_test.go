...
func generateConcreteGraphSwitch128(atePortsCount int, dutPortsCount int) *ConcreteGraph {
	var (
		nodes       []*ConcreteNode
...
func createAbstractNodesAndEdgesSwitch128(dutPortsCount int, atePortsCount int) *AbstractGraph {
...
func TestSolveTestSwitch128(t *testing.T) {
	totalNodes := 1
	atePortsCount := 58
	dutPortsCount := 70
	graph := createAbstractNodesAndEdgesSwitch128(dutPortsCount, atePortsCount)
	conGraph := generateConcreteGraphSwitch128(atePortsCount, dutPortsCount)
	startTime := time.Now()
...
// // Single switch with duts 58 prts and ate 70 ports, comment the above code and uncomment the code and execute
